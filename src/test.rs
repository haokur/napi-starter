...
fn search_index5() -> tantivy::Result<()> {
  ...
  let index = if path.exists() {
    let existing =
      Index::open_in_dir(&path).map_err(|e| Error::from_reason(format!("打开索引失败: {}", e))).unwrap();
    existing
  } else {
    Index::create_in_dir(&path, schema.clone())
      .map_err(|e| Error::from_reason(format!("创建索引失败: {}", e))).unwrap()
  };
  ...
  // 使用TermQuery对比
  let term = Term::from_field_text(body, "中文");
